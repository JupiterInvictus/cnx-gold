...
function show_module() {
	global $db, $_GET, $a, $b, $c, $d, $e, $f, $team, $sqldater, $teamdefinition, $contract, $startdate, $enddate, $startdate_month, $startdate_year, $enddate_month, $enddate_year, $enddate_day, $startdate_day, $month;
		...
		echo "</select>";
		echo "<table class='tabler sortable' id='queuelist' cellspacing=0 cellpadding=0>";
		echo "<thead><tr><th>%</th><th>Surveys</th>";
		if (($c == 'Teammate_NT_ID') && (showphotos())) {
			echo "<th></th>";
		}
		...
		if ($c){
			...
			$sql = "SELECT distinct $column_name FROM raw_data $sqldater $teamdefinition ORDER by $column_name ASC";
					...
					if($d!=''){
						...
						echo "<tr>";
						echo "<td>100%</td>";
						echo "<td>" . $surveys . "</td><td></td><td>TOTAL</td>";
						...
						while($row=$result->fetch_assoc()){
							...
							echo "<td>$surveysa</td>";
							if (($c == 'Teammate_NT_ID') && (showphotos())) {
									echo "<td class='photo' style='background-image: url(\"" . getphoto($row[$c]) . "\"); background-size: 80px 80px; background-repeat: no-repeat; background-position: center;'></td>";
							}
				...
				if($d!=''){
					if (true){
						echo "<table class=tabler width=95%>";
						echo "<thead>";
