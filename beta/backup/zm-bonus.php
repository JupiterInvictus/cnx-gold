...
function show_module() {
	global $db, $_GET, $a, $b, $c, $d, $e, $f, $team, $sqldater, $teamdefinition, $contract, $startdate, $enddate, $startdate_month, $startdate_year, $enddate_month, $enddate_year, $enddate_day, $startdate_day;
	if ($team>0){
			...
			while($row=$result->fetch_assoc()){
				...
				foreach($bd[performancepoints] as $tm => $points){
					if (guessteam($tm)!=$team){
						...
						if ($showphoto) {
							echo "<td class='photo' style='background-image: url(\"" . getphoto($tm) . "\"); background-size: 64px 64px; background-repeat: no-repeat; background-position: center;'></td>";
						}
