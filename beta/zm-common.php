...
function targetcolor($value, $contract, $metric, $team, $date, $submetric) {
	...
	if ($team){
		...
		$sql = "SELECT target_color,target_textcolor,target_value_low,target_value_high FROM targets WHERE active = 1 AND target_value_low <= '$value' AND target_value_high >= '$value-0.01' AND target_contract_id='$contract' AND target_metric_id='$metric' AND target_team_id='$team' AND target_start_date <= '$date' AND target_stop_date >= '$date' $sub LIMIT 1";
		if(!$result=$db->query($sql)){cl($sql);cl($db->error);}
		$row=$result->fetch_assoc();
		if ($row[target_color]===''){
